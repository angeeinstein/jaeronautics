...
def test_large_backlog_is_warned_about(app, monkeypatch):
    # A stalled worker shows up as a growing queue rather than an error.
    monkeypatch.setattr(diagnostics, "get_queue_summary", lambda: {
        "external_work_pending": 50, "external_work_failed": 0,
        "webhook_events_failed": 0, "webhook_events_completed": 0,
        "emails_pending": 0, "emails_exhausted": 0,
    })
    health = diagnostics.collect_system_health()
