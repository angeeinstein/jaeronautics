...
from flask_babel import Babel, _, get_locale, get_translations
# Correct import for the diagnostic check
from babel.support import NullTranslations
# --- Import db and Member model from db_models.py ---
from db_models import db, Member, User, Setting
# --- Import the new MembershipForm ---
from forms import MembershipForm, RegistrationForm, LoginForm, ChangePasswordForm, TestEmailForm
# --- Import CSRFError for custom error handling ---
...
# Database Config (using encoded password for special characters)
DB_HOST = os.getenv("DB_HOST")
DB_NAME = os.getenv("DB_NAME")
DB_USER = os.getenv("DB_USER")
# Encode password for URL to handle special characters correctly
...
# db object is now imported from db_models
babel = Babel()
...
def generate_suggested_username(member):
    ...
    # Format: {Lastname}{FirstInitial}_{StudyFieldInitial}{Year} e.g., MustermannJ_L25
...
def create_app():
    ...
    def inject_language_switcher():
        def switch_lang_url(lang):
            ...
            args['lang'] = lang
            # Fallback to 'index' if there is no endpoint (e.g., on a 404 page)
    ...
    # Pass the selector function directly to init_app as per the documentation
    babel.init_app(app, locale_selector=select_locale)

    # --- Translation Diagnostic (safe to run outside request) ---
    with app.app_context():
        from flask_babel import Translations
        import os
        path = os.path.join(app.root_path, "translations")
        t = Translations.load(path, ["de"])
        print("Loaded translations:", len(getattr(t, "_catalog", {})), "entries")
        print("Sample:", t.gettext("Joanneum Aeronautics Membership"))
    # --- End of diagnostic ---

    # --- Context Processor Fix (Expose Babel to Jinja templates & Fix URL conflicts) ---
    @app.context_processor
    def inject_babel_globals():
        """
        Makes the babel object, get_locale(), and cleaned_args available in all templates.
        This is now more robust to handle errors where the request context may be incomplete.
        """
        cleaned_args = {}
        try:
            # In some error handling scenarios (like 404), request.args might not be available.
            if request.args is not None:
                cleaned_args = {key: value for key, value in request.args.items() if key != 'lang'}
        except Exception:
            # If accessing request.args fails for any reason, default to an empty dict.
            pass
    ...
    # Apply ProxyFix for Nginx/Cloudflare Tunnel setup
    app.wsgi_app = ProxyFix(app.wsgi_app, x_for=1, x_host=1)

    # Set Stripe API key globally
    stripe.api_key = STRIPE_SECRET_KEY
    
    # --- Custom CLI Commands for Production ---
    ...
    def process_membership():
        ...
        if form.validate_on_submit():
            ...
            # Default to 'checkout' if the field was hidden and not submitted
            payment_method = form_data.pop('payment_method', 'checkout')
            
            # Force checkout if invoicing is disabled
            if settings.get('invoice_payments_enabled') != 'True':
                payment_method = 'checkout'

            try:
                if payment_method == 'checkout':
                    # --- Direct Checkout Flow (Card/SEPA) ---
                    import json
                    ...
                    # 4. Save member to DB immediately with 'unpaid' status
                    if existing_member:
    ...
    def admin():
        ...
        # --- Dynamically discover and populate email templates and senders ---
        sender_choices = []
        template_choices = []
        try:
            import json
        ...
        if request.method == 'POST':
            # This part handles the settings form
            if 'save_settings' in request.form:
                # Handle Invoice Payments Toggle
                ...
                # Handle Automatic Emails Toggle
                ...
                # Handle Automatic Email Template
        ...
        return render_template('admin/index.html', test_email_form=test_email_form, sender_choices=sender_choices, template_choices=template_choices)
    ...
    def register():
        ...
        if form.validate_on_submit():
            ...
            # All new users will now default to the 'user' role
            db.session.add(user)
            db.session.commit()
    ...
    def stripe_webhook():
        ...
        if event['type'] == 'checkout.session.completed':
            ...
            # 1. Recover member data from metadata
            ...
            try:
                ...
                if existing_member:
                    # Handle re-subscription for an inactive member
                    if not existing_member.is_active:
                        existing_member.stripe_customer_id = customer_id
                        existing_member.payment_status = 'unpaid' # Reset for the new payment cycle
                        db.session.commit()
                        app.logger.info(f"Re-subscription initiated for {email}. Awaiting payment confirmation.")
                    else:
                        # This is a duplicate for an already active member
                        app.logger.warning(f"Duplicate subscription attempt for active member: {email}. No action taken.")
                    return 'Handling existing member', 200

                # If no existing member, proceed to create a new one
                # 2. Prepare data for DB insert
                
                # Explicitly define fields to insert into the Member model
                db_fields = {
                    'salutation', 'first_name', 'last_name', 'street', 'house_number', 
                    'postal_code', 'city', 'country', 'phone_private', 'email_private', 
                    'year_group', 'title', 'phone_work', 'email_work'
                }
                
                # Map data to the model fields, providing None for missing optionals
                data_to_save = {}
                for field in db_fields:
                    value = member_data.get(field)
                    
                    # Convert empty strings from JSON back to None for nullable DB fields
                    if value == "" and field in ['title', 'phone_work', 'email_work']:
                        data_to_save[field] = None
                    else:
                        data_to_save[field] = value

                # Mandatory fields set by the system
                data_to_save['terms_accepted'] = True
                data_to_save['created_at'] = datetime.now(timezone.utc)
                
                # --- Failsafe for Stripe Customer ID ---
                customer_id = session.get('customer')
                ...
                # 3. Save to database with 'unpaid' status (now the default)
                new_member = Member(**data_to_save)
                db.session.add(new_member)
                db.session.commit()
                
                app.logger.info(f"SUCCESS: Member created with unpaid status for email: {new_member.email_private}. Session ID: {session['id']}")
        ...
        # Handle payment processing (for SEPA)
        ...
        # Handle successful payments (Card, SEPA confirmation, Invoice paid)
            ...
            if customer_id:
                member = Member.query.filter_by(stripe_customer_id=customer_id).first()
                if member:
                    # Idempotency Check: Only activate if not already active
                    if not member.is_active:
                        member.payment_status = 'paid'
                        member.is_active = True
                        db.session.commit()
                        app.logger.info(f"SUCCESS: Payment confirmed and member activated for Stripe Customer ID: {customer_id}")
                        ...
                        app.logger.info(f"Webhook received for already active member: {customer_id}. No action taken.")
                else:
                    app.logger.warning(f"Webhook for successful payment received, but no member found for Stripe Customer ID: {customer_id}")
            else:
                app.logger.warning("Webhook for successful payment received, but no Stripe Customer ID was provided.")
        ...
        # Handle failed payments
            ...
            if customer_id:
                ...
                if member:
                    ...
                    member.is_active = False # Ensure they are not active
                    db.session.commit()
                    app.logger.warning(f"Payment failed for Stripe Customer ID: {customer_id}")
            else:
                app.logger.warning("Webhook for failed payment received, but no Stripe Customer ID was provided.")
        ...
        # Handle subscription cancellations
            ...
            if customer_id:
                ...
                if member:
                    ...
                    # Differentiate cancellation reason
                    ...
                    if reason == 'payment_failed':
                        member.payment_status = 'failed'
                        db.session.commit()
                        app.logger.warning(f"Subscription for Stripe Customer ID: {customer_id} was canceled due to failed payment. Member deactivated.")
                    else:
                        # This covers manual cancellations by user or via API
                        member.payment_status = 'canceled'
                        db.session.commit()
                        app.logger.info(f"Subscription canceled for Stripe Customer ID: {customer_id}. Member deactivated.")
                else:
                    app.logger.warning(f"Webhook for subscription cancellation received, but no member found for Stripe Customer ID: {customer_id}")
                ...
                try:
                    ...
                    if customer_id:
                        ...
                        if member:
                            ...
                            db.session.commit()
                            app.logger.error(f"DISPUTE LOST for Stripe Customer ID: {customer_id}. Member has been deactivated.")
                except Exception as e:
    ...
    def handle_csrf_error(e):
        """Handles CSRF token errors with a user-friendly message."""
        flash(_('Your session has expired or the form is invalid. Please try submitting again.'), 'warning')
        return redirect(url_for('index'))

    @app.errorhandler(404)
    def page_not_found(e):
        """Handles 404 Not Found errors with a custom page."""
        return render_template('404.html'), 404

    @app.errorhandler(500)
    def internal_server_error(e):
        """Handles 500 Internal Server errors with a custom page."""
    ...
    def send_welcome_email_command(email):
        ...
        try:
            # Create a request context to allow translations and template rendering to work
            with application.test_request_context():
                ...
                if not sender_account or not template_name:
                    click.echo(click.style("Error: Email sender or template is not configured in the admin settings.", fg='red'))
                    return

                suggested_username = generate_suggested_username(member)

                # Prepare and send the email
                logo_path = os.path.join(application.root_path, 'static', 'Logo_Aeronautics_signature-logo.png')
                attachments = [{'path': logo_path, 'cid': 'logo'}]
...
# Create the final application object for Gunicorn
application = create_app()

if __name__ == "__main__":
    # Development server only - debug should be False for any semi-production testing
